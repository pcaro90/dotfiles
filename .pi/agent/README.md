...
| [web-browser](skills/web-browser/)           | Remote control of Chrome/Chromium via the Chrome DevTools Protocol (CDP). Use for JavaScript-dependent pages, clicks, forms, screenshots, authenticated browser sessions, console inspection, and network debugging. Sourced from [mitsuhiko/agent-stuff](https://github.com/mitsuhiko/agent-stuff/tree/main/skills/web-browser) with changes. |
| [web-search](skills/web-search/)             | Lightweight public web search and URL content extraction via local scripts and provider HTTP APIs. Use for ordinary search, current facts, documentation lookup, source comparison, and static/public URL-to-markdown fetches.                                                                                                                 |
...
| [notify](extensions/notify.ts)                                       | Sends desktop notifications when Pi finishes a run or needs user input. Avoids notifying while the Pi window is focused, includes a click-to-focus action on Sway, and falls back to the terminal bell when desktop notifications are unavailable.                                                                                                                                                                                        |
...
| [usage](extensions/usage.ts)                                         | Shows Codex subscription quota usage in Pi's footer when the active provider is `openai-codex`. Uses Pi's provider authentication, refreshes periodically, and preserves the last valid value during transient failures. Implementation based on [llblab/pi-codex-usage](https://github.com/llblab/pi-codex-usage).                                                                                                                          |
| [working-mode](extensions/working-mode/)                             | Controls what bash commands the AI can execute with three safety levels: 🔒 **readonly** (read-only commands only, edit/write disabled), ⚡ **normal** (read-only and trusted workflow commands auto-allowed, others ask the user with per-session "always allow" memory), 💀 **berserker** (no restrictions). Cycle with `Ctrl+Tab` or `/wmode`. Pipe-aware sub-command checking. Integrates with `ssh.ts` to hide local-only tools in remote mode.                      |

## Human Setup Notes

### web-search

The `web-search` skill reads provider credentials from its own `.env` file
instead of shell environment variables.

Initial setup:

```bash
cp ~/.pi/agent/skills/web-search/.env.example ~/.pi/agent/skills/web-search/.env
$EDITOR ~/.pi/agent/skills/web-search/.env
chmod 600 ~/.pi/agent/skills/web-search/.env
chmod +x ~/.pi/agent/skills/web-search/scripts/search.js ~/.pi/agent/skills/web-search/scripts/fetch.js
```
...
```bash
~/.pi/agent/skills/web-search/scripts/search.js "test query" -n 1
~/.pi/agent/skills/web-search/scripts/fetch.js https://example.com --max-chars 1000
```
